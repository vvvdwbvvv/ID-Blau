...
from models.losses import CharbonnierLoss, L1andPerceptualLoss, VGGPerceptualLoss
...
class Trainer:
    ...
    def _train_epoch(self):
        ...
        if self.writer and dist.get_rank() == 0:
            ...
            self.writer.add_scalar(
                "Loss/Train_lpips", total_train_lpips.avg, self.epoch
            )
            logging.info(
    ...
    def valid(self):
        ...
        if self.best_psnr < total_val_psnr.avg:
            ...
            torch.save(
                best_state,
                os.path.join(args.dir_path, "best_{}.pth".format(args.model_name)),
            )
    ...
    def save_model(self):
        ...
        if self.epoch == self.args.end_epoch:
            ...
            torch.save(
                model_state,
                os.path.join(args.dir_path, "final_{}.pth".format(args.model_name)),
            )
    ...
    def val_save_image(self, dir_path, dataset, val_num=3):
        ...
        for idx in random.sample(range(0, len(dataset)), val_num):
            sample = dataset[idx]
            blur, sharp = (
                sample["blur"].unsqueeze(0).to(device),
                sample["sharp"].unsqueeze(0).to(device),
            )
            b, c, h, w = blur.shape
            factor = 8
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--RealBlur_data_path", default="./dataset/Realblur_J", type=str
    )
    parser.add_argument(
        "--dir_path", default="./experiments/MIMO_UNetPlus_Realblur_J_GOPRO", type=str
    )
    parser.add_argument(
        "--model_name", default="MIMO_UNetPlus_Realblur_J_GOPRO", type=str
    )
    parser.add_argument(
        "--model",
        default="MIMO-UNetPlus",
        type=str,
    ...
    if args.optimizer == "adam":
        ...
        optimizer = optim.AdamW(
            net.parameters(), lr=args.init_lr, weight_decay=1e-3, betas=[0.9, 0.9]
        )
    else:
    ...
    if os.path.exists(
        ...
        if "model_state" in model_load.keys():
            model_load["model_state"] = judge_and_remove_module_dict(
                model_load["model_state"]
            )
    ...
    if args.GoPro_data_path and args.GoPro_data_path != "None":
        Train_set = GoPro_RealBlur_Loader(
            GoPro_data_path=args.GoPro_data_path,
            Realblur_data_path=args.RealBlur_data_path,
            mode="train",
            crop_size=args.crop_size,
            ZeroToOne=False,
        )
    else:
        Train_set = RealBlur_Loader(
            data_path=args.RealBlur_data_path,
            mode="train",
            crop_size=args.crop_size,
            ZeroToOne=False,
        )
    ...
    trainer = Trainer(
        dataloader_train, dataloader_val, net, optimizer, scheduler, args, writer
    )
    trainer.train()
