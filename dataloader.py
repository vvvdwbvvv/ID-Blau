...
class Flow_Loader(Dataset):
    def __init__(self, data_path, flow_path, mode, crop_size=None, flow_norm=True):
        ...
        for video in sorted(os.listdir(os.path.join(data_path, mode))):
            ...
            flow_video_data_path = sorted(
                glob.glob(os.path.join(flow_video_path, "*.npy"))
            )
            self.flow_list.extend(flow_video_data_path)
            self.blur_list.extend(
                [
                    npy_path.replace(flow_video_path, data_blur_video_path).replace(
                        ".npy", ".png"
                    )
                    for npy_path in flow_video_data_path
                ]
            )
            self.sharp_list.extend(
                [
                    npy_path.replace(flow_video_path, data_sharp_video_path).replace(
                        ".npy", ".png"
                    )
                    for npy_path in flow_video_data_path
                ]
...
class Multi_GoPro_Loader(Dataset):
    def __init__(
        ...
        crop_size=None,
        ZeroToOne=False,
        video_generate_path=None,
    ):
        """generate_path can be str or list"""
        assert data_path or generate_path or video_generate_path, (
            "must have one dataset path !"
        )
        self.blur_list = []
        self.sharp_list = []
        ...
        if data_path:
            for video in sorted(os.listdir(os.path.join(data_path, mode))):
                self.blur_list.extend(
                    sorted(
                        glob.glob(os.path.join(data_path, mode, video, "blur", "*.png"))
                    )
                )
                self.sharp_list.extend(
                    sorted(
                        glob.glob(
                            os.path.join(data_path, mode, video, "sharp", "*.png")
                        )
                    )
                )
        ...
        if generate_path and mode == "train":
            ...
            for now_generate_path in generate_path:
                sharp_image_folders_list = sorted(
                    os.listdir(os.path.join(now_generate_path, "sharp"))
                )
                for folder in sharp_image_folders_list:
                    blur_images_list = sorted(
                        glob.glob(
                            os.path.join(now_generate_path, "blur", folder, "*.png")
                        )
                    )
                    ...
                    sharp_images_list = (
                        glob.glob(
                            os.path.join(
                                now_generate_path, "sharp", folder, "sharp.png"
                            )
                        )
                    ) * blur_length
        ...
        if video_generate_path and mode == "train":
            ...
            for now_generate_path in video_generate_path:
                video_image_folders_list = sorted(
                    os.listdir(os.path.join(now_generate_path, "sharp"))
                )
                for video in video_image_folders_list:
                    sharp_image_folders_list = sorted(
                        os.listdir(os.path.join(now_generate_path, "sharp", video))
                    )
                    for folder in sharp_image_folders_list:
                        blur_images_list = sorted(
                            glob.glob(
                                os.path.join(
                                    now_generate_path, "blur", video, folder, "*.png"
                                )
                            )
                        )
                        self.blur_list.extend(blur_images_list)
...
class RealBlur_Loader(Dataset):
    def __init__(self, data_path=None, mode="train", crop_size=None, ZeroToOne=False):
        ...
        for video in sorted(os.listdir(os.path.join(data_path, mode, "blur"))):
            self.blur_list.extend(
                sorted(glob.glob(os.path.join(data_path, mode, "blur", video, "*.png")))
            )
            self.sharp_list.extend(
                sorted(
                    glob.glob(os.path.join(data_path, mode, "sharp", video, "*.png"))
                )
            )
...
class GoPro_RealBlur_Loader(Dataset):
    def __init__(
        self,
        GoPro_data_path=None,
        Realblur_data_path=None,
        mode="train",
        crop_size=None,
        ZeroToOne=False,
    ):
        ...
        for video in sorted(os.listdir(os.path.join(GoPro_data_path, mode))):
            self.blur_list.extend(
                sorted(
                    glob.glob(
                        os.path.join(GoPro_data_path, mode, video, "blur", "*.png")
                    )
                )
            )
            self.sharp_list.extend(
                sorted(
                    glob.glob(
                        os.path.join(GoPro_data_path, mode, video, "sharp", "*.png")
                    )
                )
            )
        ...
        for video in sorted(os.listdir(os.path.join(Realblur_data_path, mode, "blur"))):
            self.blur_list.extend(
                sorted(
                    glob.glob(
                        os.path.join(Realblur_data_path, mode, "blur", video, "*.png")
                    )
                )
            )
            self.sharp_list.extend(
                sorted(
                    glob.glob(
                        os.path.join(Realblur_data_path, mode, "sharp", video, "*.png")
                    )
                )
            )
...
class Test_Loader(Dataset):
    def __init__(self, data_path=None, crop_size=None, ZeroToOne=False):
        ...
        if crop_size:
            self.transform = transforms.Compose(
                [RandomCrop(crop_size, crop_size), Normalize(ZeroToOne), ToTensor()]
            )
        else:
            self.transform = transforms.Compose([Normalize(ZeroToOne), ToTensor()])
        ...
        if data_path:
            self.blur_list.extend(
                sorted(glob.glob(os.path.join(data_path, "input", "*.png")))
            )
            if self.is_sharp_dir:
                self.sharp_list.extend(
                    sorted(glob.glob(os.path.join(data_path, "target", "*.png")))
                )
