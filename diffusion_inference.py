...
@torch.no_grad()
def valid(
    model, dataloader_val, sample_timesteps, device, valid_iters=None, title=None
):
    model.eval()
    ...
    for idx, sample in enumerate(tq):
        ...
        tq.set_postfix(
            LPIPS=total_val_lpips.avg, PSNR=total_val_psnr.avg, NIQE=total_val_niqe.avg
        )
    end_time = time.time()
    ...
    logging.info(
        f"PSNR : {total_val_psnr.avg:.4f}, LPIPS : {total_val_lpips.avg:.4f}, NIQE : {total_val_niqe.avg:.4f}"
    )


def val_save_image(
    model, dir_path, dataset, sample_timesteps, val_num=3, val_idxs=None
):
    ...
    with torch.no_grad():
        ...
        for i, idx in enumerate(val_idxs):
            ...
            if args.model == "FlowMatching":
                ...
                output = model.sample(
                    condition=condition, device=device, tqdm_visible=True
                )
            output = output.clamp(-0.5, 0.5)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", default="train", type=str, choices=["train", "test"]
    )
