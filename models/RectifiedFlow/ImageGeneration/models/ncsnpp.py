...
class NCSNpp(nn.Module):
  ...
  def __init__(self, config):
    ...
    channels = getattr(config.data, 'input_channels', config.data.num_channels)
    output_channels = getattr(config.data, 'output_channels', config.data.num_channels)
      ...
      if progressive != 'none':
        if i_level == num_resolutions - 1:
          ...
          if progressive == 'output_skip':
            modules.append(nn.GroupNorm(num_groups=min(in_ch // 4, 32),
                                        num_channels=in_ch, eps=1e-6))
            modules.append(conv3x3(in_ch, output_channels, bias=True, init_scale=init_scale))
            pyramid_ch = output_channels
          elif progressive == 'residual':
