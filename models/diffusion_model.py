...
class ResidualBlock(nn.Module):
    def __init__(
        self, in_channels, out_channels, dropout, time_dim=None, activatedfun=nn.SiLU
    ):
        super().__init__()
        self.time_dim = time_dim
        self.layer1 = nn.Sequential(
            activatedfun(), nn.Conv2d(in_channels, out_channels, 3, padding=1)
        )
...
class UNet(nn.Module):
    def __init__(
        ...
        self.last_layer = nn.Sequential(
            activatedfun(), nn.Conv2d(base_channels, 3, 3, padding=1)
        )
